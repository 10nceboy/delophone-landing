import{g as E,f as o,h as g}from"./menu-d5da2a40.js";const C=e=>{localStorage.setItem("cartArray",JSON.stringify(e))},h=()=>JSON.parse(localStorage.getItem("cartArray"))||[],x={mobile:"Мобильные",499:"Москва (499)",495:"Москва (495)",49599:"Москва 499+495 (комплект)",8800:"8 800",sochi:"Сочи",spb:"Санкт-Петербург",kras:"Краснодар"},q=[{id:"1",type:"single",group:"499",code1:"8 495",code2:"",number:"2434561",price:"990 ₽ разово",abonprice:"190 ₽/мес"},{id:"2",type:"pair",group:"49599",code1:"8 495",code2:"8 499",number:"2564561",price:"1 990 ₽ разово",abonprice:""},{id:"3",type:"vip",group:"495",code1:"8 495",code2:"",number:"7277777",price:"450 000 ₽",abonprice:""},{id:"4",type:"vip",group:"sochi",code1:"8 495",code2:"",number:"7777777",price:"450 000 ₽",abonprice:""},{id:"5",type:"pair",group:"49599",code1:"8 495",code2:"8 499",number:"7377777",price:"150 000 ₽",abonprice:""},{id:"6",type:"single",group:"mobile",code1:"8 988",code2:"",number:"2934566",price:"990 ₽ разово",abonprice:"190 ₽/мес"},{id:"7",type:"single",group:"kras",code1:"8 861",code2:"",number:"2434567",price:"990 ₽ разово",abonprice:"190 ₽/мес"},{id:"8",type:"single",group:"495",code1:"8 495",code2:"",number:"2434568",price:"990 ₽ разово",abonprice:"190 ₽/мес"},{id:"9",type:"single",group:"495",code1:"8 495",code2:"",number:"4434569",price:"990 ₽",abonprice:"190 ₽/мес"},{id:"10",type:"single",group:"8800",code1:"8 800",code2:"",number:"1233567",price:"990 ₽",abonprice:"190 ₽/мес"}],l=document.querySelector(".cart__content-upside"),_=document.querySelector(".choose__phones-cards"),A=document.querySelector(".cart__summary"),u=document.querySelector(".cart__button-buy"),$=document.querySelector(".cart__header"),d=document.querySelector(".cart__mobile-quantity");let r=[];const f=()=>`
    <svg
  ...
  >
    ...
    <path
      ...
      fill="#9CA5AC"
    />
  </svg>`,p={single:"single",pair:"pair",vip:"vip"},I=()=>{q.forEach(e=>{switch(e.type){case p.single:_.innerHTML+=`<div class="card card_white choose__phones-card" data-id="${e.id}">
      <div class="choose__phones-single-number no-select">
    ...
    </div> `;break;case p.pair:_.innerHTML+=`
        ...
        `;break;case p.vip:_.innerHTML+=`<div class="card card_white choose__phones-card" data-id ="${e.id}" >
        ...
        </div>`;break}})},T=e=>{if(r.some(t=>t.id===e))return!1;{const t=q.find(s=>s.id===e);return r.push(t),t}},S=e=>{const t=r.findIndex(c=>c.id===e);if(t<0)return;const s=l.querySelectorAll(".cart__item")[t];s.classList.add("cart__item_deleted"),s.addEventListener("animationend",()=>{r.splice(t,1),m(),C(r)},{once:!0})},M=e=>`<div class="cart__content-header no-select">${x[e]??""}</div>`;function L(e){var c,i,a;const{target:t}=e,s=(i=(c=t.parentElement)==null?void 0:c.parentElement)==null?void 0:i.dataset.cartItemId;(a=document.querySelector(`.choose__phones-card_disabled[data-id="${s}"]`))==null||a.classList.remove("choose__phones-card_disabled"),s&&S(s)}const m=()=>{r=E(r,"group");let e=null,t="";r.length==0&&(location.pathname.includes("order")?t=` <div class="cart__content-header">
...
покупки.
</div>`);let s=l.querySelectorAll(".cart__item-icon");s.forEach(c=>c.removeEventListener("click",L)),r.forEach(({id:c,number:i,code1:a,code2:b,group:n,abonprice:v,price:y,type:w})=>{e!==n&&(e=n,t+=M(n)),w!=="pair"?t+=`<div class="card card_white cart__item" data-cart-item-id=${c}>
          <div class="cart__item-number no-select">
          <span class="choose__city-code">${a}</span> ${i.substring(0,3)}-${i.substring(3,5)}-${i.substring(5,7)}
 </div>
 <div class="cart__item-price no-select">${y}<div class="cart__item-price_per-month no-select">${v}</div>
 </div>${f()}</div>`:t+=`<div class="card card_white cart__item" data-cart-item-id=${c}>
      <div class="cart__item-pair-numbers no-select">
        <div class="cart__item-number">
        <span class="choose__city-code">${a}</span> ${i.substring(0,3)}-${i.substring(3,5)}-${i.substring(5,7)}
        </div>
        <div class="cart__item-number">
        <span class="choose__city-code">${b}</span> ${i.substring(0,3)}-${i.substring(3,5)}-${i.substring(5,7)}
        </div>
      </div>
      <div class="cart__item-price no-select">${y}</div>
      <div class="cart__item-price_per-month no-select">${v}</div>
      ${f()}
      </div>`}),l.innerHTML=t,s=l.querySelectorAll(".cart__item-icon"),s.forEach(c=>{c.addEventListener("click",L)}),H(),k()},k=()=>{let e=0,t=0;r.forEach(c=>{e+=parseInt(c.price.split(" ").join(""),10),t+=parseInt(c.abonprice.split(" ").join(""),10)||0}),A.innerHTML=`
  <div class="cart__summary">
    <span class="cart__summary_once">${o(e)} ₽,</span>
    <span class="cart__summary_per-month"> ${o(t)} ₽/мес </span>
  </div>`;const s=document.querySelector(".order__pay-once");if(s){const c=document.querySelector(".order__pay-per-month"),i=document.querySelector(".order__summary");s.textContent=o(e),c.textContent=o(t),i.textContent=o(t+e),document.querySelector(".order__summary-button").textContent=document.querySelector(".order__summary").textContent}},H=()=>{var t,s;let e=r.length;$.innerHTML=`
...
`,$.classList.add("no-select"),(d||u)&&(e==0?(u.disabled=!0,d.classList.remove("cart__mobile-quantity_active"),(t=document.querySelector(".cart__mobile-button"))==null||t.classList.remove("cart__mobile-button_active")):e==1&&(u.textContent="Купите номер телефона"),e>0?(u.disabled=!1,d.classList.add("cart__mobile-quantity_active"),d.textContent=e,(s=document.querySelector(".cart__mobile-button"))==null||s.classList.add("cart__mobile-button_active")):d.classList.remove("cart__mobile-quantity_active"))},P=()=>{location.pathname.includes("order")?r=h():(r=h(),I(),document.querySelectorAll(".choose__phones-card").forEach(t=>{const s=t.dataset.id;r.some(c=>c.id===s)&&t.classList.add("choose__phones-card_disabled"),t.dispatchEvent(new MouseEvent("mousedown")),t.dispatchEvent(new MouseEvent("mouseup")),t.addEventListener("click",c=>{if(!T(s)){t.classList.remove("choose__phones-card_disabled"),S(s);return}t.classList.add("choose__phones-card_disabled"),m();const a=r.findIndex(n=>n.id===s);l.querySelectorAll(".cart__item")[a].classList.add("cart__item_new"),C(r)})})),m()};P();
