  ...
  >
    ...
    />
      ...
      <div class="choose__phones-single-number no-select">
        ...
        </div>`;break}})},k=e=>{if(c.some(t=>t.id===e))return!1;{const t=S.find(s=>s.id===e);return c.push(t),t}},x=e=>{const t=c.findIndex(r=>r.id===e);if(t<0)return;const s=d.querySelectorAll(".cart__item")[t];s.classList.add("cart__item_deleted"),c.splice(t,1),q(c),s.addEventListener("animationend",()=>{v()},{once:!0})},H=e=>`<div class="cart__content-header no-select">${A[e]??""}</div>`;function C(e){var r,i,n;const{target:t}=e,s=(i=(r=t.parentElement)==null?void 0:r.parentElement)==null?void 0:i.dataset.cartItemId;(n=document.querySelector(`.choose__phones-card_disabled[data-id="${s}"]`))==null||n.classList.remove("choose__phones-card_disabled"),s&&x(s)}const v=()=>{c=E(c,"group");let e=null,t="";c.length==0&&(location.pathname.includes("order")?t=` <div class="cart__content-header">
...
покупки.
</div>`);let s=d.querySelectorAll(".cart__item-icon");s.forEach(r=>r.removeEventListener("click",C)),c.forEach(({id:r,number:i,code1:n,code2:u,group:p,abonprice:y,price:g,type:w})=>{e!==p&&(e=p,t+=H(p)),w!=="pair"?t+=`<div class="card card_white cart__item" data-cart-item-id=${r}>
 ...
 <div class="cart__item-price no-select">${g}<div class="cart__item-price_per-month no-select">${y}</div>
 </div>${L()}</div>`:t+=`<div class="card card_white cart__item" data-cart-item-id=${r}>
      ...
      ${L()}
      </div>`}),d.innerHTML=t,s=d.querySelectorAll(".cart__item-icon"),s.forEach(r=>{r.addEventListener("click",C)}),P(),M()},M=()=>{let e=0,t=0;c.forEach(r=>{e+=parseInt(r.price.split(" ").join(""),10),t+=parseInt(r.abonprice.split(" ").join(""),10)||0}),I.innerHTML=`
  <div class="cart__summary">
    <span class="cart__summary_once">${o(e)} ₽,</span>
    <span class="cart__summary_per-month"> ${o(t)} ₽/мес </span>
  </div>`;const s=document.querySelector(".order__pay-once");if(s){const r=document.querySelector(".order__pay-per-month"),i=document.querySelector(".order__summary");s.textContent=o(e),r.textContent=o(t),i.textContent=o(t+e),document.querySelector(".order__summary-button").textContent=document.querySelector(".order__summary").textContent}},P=()=>{let e=c.length;f.innerHTML=`
...
`,f.classList.add("no-select"),(a||l)&&(e==0?(l.disabled=!0,a.classList.remove("cart__mobile-quantity_active"),_.classList.remove("cart__mobile-button_active")):e==1&&(l.textContent="Купите номер телефона"),e>0?(l.disabled=!1,a.classList.add("cart__mobile-quantity_active"),a.textContent=e,_.classList.add("cart__mobile-button_active")):(a.classList.remove("cart__mobile-quantity_active"),_.classList.remove("cart__mobile-button_active")))},B=()=>{location.pathname.includes("order")?c=$():(c=$(),T(),document.querySelectorAll(".choose__phones-card").forEach(t=>{const s=t.dataset.id;c.some(r=>r.id===s)&&t.classList.add("choose__phones-card_disabled"),t.addEventListener("click",()=>{if(!k(s)){t.classList.remove("choose__phones-card_disabled"),x(s),c.length!==0?a.textContent=c.length:(console.log(c.length),a.textContent=c.length,a.classList.remove("cart__mobile-quantity_active"),_.classList.remove("cart__mobile-button_active"));return}t.classList.add("choose__phones-card_disabled"),v();const i=c.findIndex(u=>u.id===s);d.querySelectorAll(".cart__item")[i].classList.add("cart__item_new"),q(c)})})),v()};B();
