import { transitionEnter, transitionLeave } from '../utils/dom';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  expand.addEventListener('click', () => {
    ...
    if (state) {
      ...
      expandContent.forEach((element) =>
        transitionEnter(element, 'expand__content')
      );
      tableHeader.forEach((element) =>
        transitionEnter(element, 'phone-numbers__header')
      );
      phonenNumber.scrollIntoView({ block: 'start', behavior: 'smooth' });
      smoothScrollSafari(phonenNumber);
    } else {
      expandContent.forEach((element) =>
        transitionLeave(element, 'expand__content')
      );
      tableHeader.forEach((element) =>
        transitionLeave(element, 'phone-numbers__header')
      );
      window.setTimeout(() => {
